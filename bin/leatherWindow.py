...
class LeatherWindow_preview(tk.Frame):
		...
		for event in pygame.event.get():
			if event.type == pygame.MOUSEWHEEL:
				...
				if event.button == 1:
					...
					self.y_layer_items_offset = []
					self.r_layer_items_offset = []
					for old_point, new_point, history in zip(self.temp_c_layer_items, self.displayed_c_layer_items, self.c_layer_items_calc_history):
						history[0].append(old_point[0] - new_point[0])
						history[1].append(old_point[1] - new_point[1])
					for old_item, new_item, history in zip(self.temp_h_layer_items, self.displayed_h_layer_items, self.h_layer_items_calc_history):
					...
					for old_item, new_item, history in zip(self.temp_b_layer_items, self.displayed_b_layer_items, self.b_layer_items_calc_history):
						for old_point, new_point, point_history in zip(old_item, new_item, history):
							point_history[0].append(old_point[0] - new_point[0])
							point_history[1].append(old_point[1] - new_point[1])
					for old_item, new_item, history in zip(self.temp_g_layer_items, self.displayed_g_layer_items, self.g_layer_items_calc_history):
						for old_point, new_point, point_history in zip(old_item, new_item, history):
							point_history[0].append(old_point[0] - new_point[0])
							point_history[1].append(old_point[1] - new_point[1])
					for old_item, new_item, history in zip(self.temp_y_layer_items, self.displayed_y_layer_items, self.y_layer_items_calc_history):
						for old_point, new_point, point_history in zip(old_item, new_item, history):
							point_history[0].append(old_point[0] - new_point[0])
							point_history[1].append(old_point[1] - new_point[1])
					for old_item, new_item, history in zip(self.temp_r_layer_items, self.displayed_r_layer_items, self.r_layer_items_calc_history):
						for old_point, new_point, point_history in zip(old_item, new_item, history):
							point_history[0].append(old_point[0] - new_point[0])
							point_history[1].append(old_point[1] - new_point[1])
	...
	def calculate_position(self):
		...
		for offset, history, old_point in zip(self.c_layer_items_pos_offset, self.c_layer_items_calc_history, self.temp_c_layer_items):
		...
		for item, item_offset, history, old_item in zip(self.displayed_h_layer_items, self.h_layer_items_pos_offset, self.h_layer_items_calc_history, self.temp_h_layer_items):
			item_list = []
		...
		self.displayed_h_layer_items = new_h_layer_items
		for item, item_offset, history, old_item in zip(self.displayed_b_layer_items, self.b_layer_items_pos_offset, self.b_layer_items_calc_history, self.temp_b_layer_items):
			item_list = []
			for point, offset, point_history, old_point in zip(item, item_offset, history, old_item):
				item_list.append([round((offset[0] + screen_center[0]),1), round(offset[1] + screen_center[1],1)])
				point_history[0].append(round((old_point[0] - (offset[0] + screen_center[0])),1))
		...
		self.displayed_b_layer_items = new_b_layer_items
		for item, item_offset, history, old_item in zip(self.displayed_g_layer_items, self.g_layer_items_pos_offset, self.g_layer_items_calc_history, self.temp_g_layer_items):
			item_list = []
			for point, offset, point_history, old_point in zip(item, item_offset, history, old_item):
				item_list.append([round((offset[0] + screen_center[0]),1), round(offset[1] + screen_center[1],1)])
				point_history[0].append(round((old_point[0] - (offset[0] + screen_center[0])),1))
		...
		self.displayed_g_layer_items = new_g_layer_items
		for item, item_offset, history, old_item in zip(self.displayed_y_layer_items, self.y_layer_items_pos_offset, self.y_layer_items_calc_history, self.temp_y_layer_items):
			item_list = []
			for point, offset, point_history, old_point in zip(item, item_offset, history, old_item):
				item_list.append([round((offset[0] + screen_center[0]),1), round(offset[1] + screen_center[1],1)])
				point_history[0].append(round((old_point[0] - (offset[0] + screen_center[0])),1))
		...
		self.displayed_y_layer_items = new_y_layer_items
		for item, item_offset, history, old_item in zip(self.displayed_r_layer_items, self.r_layer_items_pos_offset, self.r_layer_items_calc_history, self.temp_r_layer_items):
			item_list = []
			for point, offset, point_history, old_point in zip(item, item_offset, history, old_item):
				item_list.append([round((offset[0] + screen_center[0]),1), round(offset[1] + screen_center[1],1)])
				point_history[0].append(round((old_point[0] - (offset[0] + screen_center[0])),1))
	...
	def calculation_history(self):
		pass
		###  CALC TEST  ###
		#if type(self.c_layer_items_calc_history[0]) != 'None':
		#	x_point_story = self.c_layer_items_calc_history[0][0]
		#	y_point_story = self.c_layer_items_calc_history[0][1]
#
		#	x_pierwotne = self.c_layer_items[0][0]
		#	y_pierwotne = self.c_layer_items[0][1]
#
		#	x_wyswietlone = self.displayed_c_layer_items[0][0]
		#	y_wyswietlone = self.displayed_c_layer_items[0][1]
#
...
#
		...
		#			x_obliczone = temp_y
#
		#	print('calc_history: ', self.c_layer_items_calc_history[0])
		#	print('porownanie x old-calc-displayed', x_pierwotne, x_obliczone, x_wyswietlone)
		#	print('porownanie y old-calc-displayed', y_pierwotne, y_obliczone, y_wyswietlone)
		### DLA C DZIALA ###
		...
		if type(self.b_layer_items_calc_history[0][0]) != 'None':
			...
			print('porownanie prev x old-calc-displayed', x_pierwotne, x_obliczone, x_wyswietlone)
			print('porownanie prev y old-calc-displayed', y_pierwotne, y_obliczone, y_wyswietlone)
		###   DLA INNYCH TEZ DZIALA   ###
###############################################################################################
###############################################################################################
###############################################################################################
