...
while True:
	...
	sec_x = None
	thumb_x = None
	for index, hand, landmarks_list in zip(range(0, len(detection_result.handedness)), detection_result.handedness, hand_landmarks_list):
		if 'Left' in str(hand):
	...
	for idx in range(len(hand_landmarks_list)):
		...
		for id, landmark in enumerate(landmarks):
			...
			if thumb_x != None and sec_x != None:
				...
				if math.sqrt(calc_x*calc_x+calc_y*calc_y) < 40:
